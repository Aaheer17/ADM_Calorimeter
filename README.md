...
To train the layer model and generate samples, follow these commands:

First, you will need to create the training config files. This can be generated by running this command. Make sure to keep the file path and folder name consistent. 
```bash
python config_creation.py
```
Second, run the submit_all.sh file. Make sure to keep the file name consistent.
```bash
./submit_all.sh
```
